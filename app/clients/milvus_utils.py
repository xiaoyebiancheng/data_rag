...
def delete_by_doc_id(client, collection_name: str, doc_id: str) -> None:
...
def delete_by_file_title_version(client, collection_name: str, file_title: str, version: int) -> None:
    if client is None or not collection_name or not file_title:
        return
    safe_file_title = escape_milvus_string(file_title)
    client.delete(
        collection_name=collection_name,
        filter=f'file_title=="{safe_file_title}" and version=={int(version)}'
    )
    client.load_collection(collection_name=collection_name)


def delete_by_item_name(client, collection_name: str, item_name: str) -> None:
    if client is None or not collection_name or not item_name:
        return
...
def delete_by_file_title(client, collection_name: str, file_title: str) -> None:
    if client is None or not collection_name or not file_title:
        return
    safe_file_title = escape_milvus_string(file_title)
    client.delete(collection_name=collection_name, filter=f'file_title=="{safe_file_title}"')
    client.load_collection(collection_name=collection_name)
