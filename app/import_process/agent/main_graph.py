...
def route_after_entry(state: ImportGraphState) -> str:
    ...
    """
    # 优化: 优化的原因是重复上传在入口已可判定，没有必要继续消耗PDF解析、切片和向量化资源。
    if state.get("document_status") == "DUPLICATED":
        return END
...
if __name__ == "__main__":
    ...
    if not os.path.exists(test_pdf_path):
        ...
        # 使用统一默认状态构造，避免后续节点访问缺省字段时出现KeyError
        test_state = create_default_state(
            ...
            is_pdf_read_enabled=False,  # 由node_entry根据后缀动态设置
        ...
        try:
            ...
            final_state = None
            for step in kb_import_app.run(test_state, stream_mode="updates"):
                # updates 模式返回 {node_name: patch}，可准确拿到执行节点名
                if step:
                    current_node = next(iter(step.keys()))
                    logger.info(f"✅ 节点执行完成：{current_node}")
                else:
                    logger.info("✅ 节点执行完成：未知节点")
                # 每轮基于patch更新完整状态，方便最终结果统计
                test_state.update(next(iter(step.values()), {}))
            ...
            logger.exception(f"===== 全流程测试运行失败 =====") #能够打印出具体哪一行出错
